...
int initialize() {
  ...
  if (validator::errors) {
    logger::config.info(std::to_string(validator::errors) + " error(s) generated in total when validating Default Config.");
    logger::config.info("Choose any of these methods to contact Daiari (please describe your situation and provide this file):");
    logger::config.info(AUTHOR_CONTACT);
    return 1;
  }

  loader::reset_errors(); loader::load_ini_from_file(config_path, config);
  if (loader::errors || loader::warns) {
    logger::config.info(std::to_string(loader::errors) + " error(s) and " + std::to_string(loader::warns) + " warning(s) generated in total when loading Config.");
    logger::config.info("You should check mentioned lines in '" + config_path + "'.");
    logger::config.info("If you have no clue, choose any of these methods to contact Daiari (please describe your situation and provide this file):");
    logger::config.info(AUTHOR_CONTACT);
  }
  ...
  if (user_validator::warns) {
    logger::config.info(std::to_string(user_validator::warns) + " warning(s) generated in total when validating Config.");
    logger::config.info("You should check mentioned keys in '" + config_path + "'.");
    logger::config.info("If you have no clue, choose any of these methods to contact Daiari (please describe your situation and provide this file):");
    logger::config.info(AUTHOR_CONTACT);
    return 4;
  }
