...
void prepare() {
  ...
  if (!font.openFromFile("res/font.otf")) logger::interface.error("Cannot open default font file res/font.otf.");
}
void draw_step_text(std::string step) {
  sf::Text text(font, step, 32); text.setFillColor(sf::Color::White);
...
void render_temp(game &g) {
  ...
  g.print(ss);
  sf::Text text(font, ss.str(), 12 * config::get<double>("demo.sizemult")); text.setFillColor(sf::Color::White);
